...
import { useBasketballSocket } from '../hooks/useBasketballSocket';
import { TimerBar } from '../components/TimerBar';

export function BasketballScoringPage() {
  const { matchId } = useParams<{ matchId: string }>();
  const { connected, matchState, startMatch, recordBasket, undoLastBasket, endQuarter, emitTimer } =
    useBasketballSocket(matchId ?? null);
  const [players, setPlayers] = useState<Player[]>([]);
  const [teamNames, setTeamNames] = useState<Record<string, string>>({});
  const [scoringTeam, setScoringTeam] = useState<'home' | 'away'>('home');
  const [selectedScorer, setSelectedScorer] = useState('');
  const [selectedAssist, setSelectedAssist] = useState('');
  const [points, setPoints] = useState<2 | 3>(2);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <header style={{ padding: '0.75rem 1rem', background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontWeight: 700 }}>Basketball Scorer · Q{matchState.currentQuarter}</span>
      ...
      {matchState.status === 'quarter-break' ? (
        ...
        <div style={{ flex: 1, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button onClick={() => setPoints(2)} style={{ flex: 1, padding: '0.75rem', background: points === 2 ? 'var(--accent)' : 'var(--surface-elevated)', color: points === 2 ? '#0a0e12' : 'var(--text)', borderRadius: 8, fontWeight: 600 }}>2 points</button>
            <button onClick={() => setPoints(3)} style={{ flex: 1, padding: '0.75rem', background: points === 3 ? 'var(--accent)' : 'var(--surface-elevated)', color: points === 3 ? '#0a0e12' : 'var(--text)', borderRadius: 8, fontWeight: 600 }}>3 points</button>
          </div>
          ...
          <button
            ...
            onClick={() => {
              recordBasket(teamId, selectedScorer, points, selectedAssist || undefined);
              setSelectedScorer('');
              setSelectedAssist('');
            }}
          ...
          <button onClick={undoLastBasket} style={{ padding: '1rem', background: 'var(--surface-elevated)', color: 'var(--text)', borderRadius: 8, border: '1px solid var(--border)' }}>
            Undo Last Basket
          </button>
