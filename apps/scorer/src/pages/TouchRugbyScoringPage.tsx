...
import { useTouchRugbySocket } from '../hooks/useTouchRugbySocket';
import { TimerBar } from '../components/TimerBar';

export function TouchRugbyScoringPage() {
  const { matchId } = useParams<{ matchId: string }>();
  const { connected, matchState, startMatch, recordTry, undoLastTry, endHalf, emitTimer } = useTouchRugbySocket(matchId ?? null);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <header style={{ padding: '0.75rem 1rem', background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontWeight: 700 }}>Touch Rugby · Half {matchState.currentHalf}</span>
      ...
      {matchState.status === 'half-time' ? (
        ...
        <div style={{ flex: 1, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <button
            ...
            onClick={() => { recordTry(teamId, selectedScorer); setSelectedScorer(''); }}
            style={{ padding: '1.5rem', fontSize: '1.5rem', fontWeight: 800, background: 'var(--accent)', color: '#0a0e12', borderRadius: 12 }}
          >
            TRY
          </button>
          <button onClick={undoLastTry} style={{ padding: '1rem', borderRadius: 8, border: '1px solid var(--border)' }}>
            Undo Last Try
          </button>
